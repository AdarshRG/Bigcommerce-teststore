    ...
    addProductToCart(event, form) {
        ...
        utils.api.cart.itemAdd(normalizeFormData(new FormData(form)), (err, response) => {
            ...
            if (this.previewModal) {     
                ...
                var navUserItemCart = document.querySelector('.navUser-item--cart a');
                var customerInput = document.getElementById('customerr');
                var customerId = customerInput.getAttribute('data');
                if (customerId) {
                  console.log("Customer ID:", customerId);          
                  navUserItemCart.click();
                } else {
                  alert("Please Login");
                  window.location.href = 'http://localhost:3001/login.php?action=create_account';
                }
